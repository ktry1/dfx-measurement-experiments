...
module {
  ...
  public func scan(db : DB, { skLowerBound; skUpperBound; limit; ascending } : ScanOptions) : ScanResult {
    ...
    {
      entities = Array.map<(E.SK, E.AttributeMap), E.Entity>(results, func((sk, attributeMap)) { { pk = db.pk; sk = sk; attributes = attributeMap } });
      nextKey = nextKey;
    };
  };
};
