...
actor class FileStorage(is_prod : Bool) = this {
	...
	//Custom function to get the assets without using HTTP
	public shared ({caller}) func getChunk(id: Asset_ID, index: Chunk_ID) : async Result.Result<Blob, Text> {
		switch (Map.get(assets, thash, id)) {
			case (?asset) {
				if (asset.owner == Principal.toText(caller)) {
					//returning the chunk
					switch (asset.content) {
						case (null) {return #err("Asset empty")};
						case (?content) {return #ok(content[index])}
					}
				} else {
