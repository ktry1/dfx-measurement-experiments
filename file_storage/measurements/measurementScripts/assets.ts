...
import { idlFactory as managerIdlFactory, assets } from "../../src/declarations/assets";
...
async function main() {
    ...
    const actor: typeof assets = makeActor(agent, managerIdlFactory, canisterId);
    ...
    for (let file of testFiles) {
        ...
        await executeCommand("dfx", ["canister", "call", canisterName, "authorize",
...
async function uploadAsset(file_path: string, chunkSize: number, actor: typeof assets) : Promise<RtsData> {
...
async function readAsset(key: string, chunkSize: bigint, actor: typeof assets) : Promise<RtsData> {
...
async function deleteAsset(key: string, actor: typeof assets) : Promise<RtsData> {
    let total_rts_usage = empty_rts_data;
...
async function updateAsset(file_path: string, chunkSize: number, actor: typeof assets) : Promise<RtsData> {
    let total_rts_usage = empty_rts_data;
    let prev_rts_data = empty_rts_data;
    let new_rts_data = empty_rts_data;
